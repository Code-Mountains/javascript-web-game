...
;(function ( $, window, document, undefined ) {
		...
		$.extend(Plugin.prototype, {
				...
				dpr: function(){
					var dprVar =  this.settings.fruitFall.settings.dpr;
					if(dprVar>1.2){
		               dprVar=2;
		            }
		            return dprVar;
				...
				initScreen: function(){
					...
					this.playButton.click = this.playButton.touchstart = function(mouseData) {
					...
					this.replayButton.click =this.replayButton.touchstart = function(mouseData) {
						ref.settings.fruitFall.soundFX.play('pencilStrike2');
						...
						TweenMax.to(ref.highScoreList, 0.5,{alpha:0});
				...
				preloader : function(doWhat){
		         ...
		         randomSwirlsAndDrips: function(_x,_y){
		          ...
		          animation.position.x = _x;
		          animation.position.y = _y;

		          animation.scale.x=animation.scale.y = Math.random()*1+0.5;
		         ...
		         submitScore: function(){
		         	...
		         	//Supply it with a string representing the player's name, score and a secret key
		         	var _name = this.playerName.text;
		         	var _score = this.highScoreText.text;
		         	var secretKey = 'fruitFall98324GME';
		         	var _hash=CryptoJS.MD5(_name+_score+secretKey);

		         	var ref=this;

		         	$.ajax({
		         		url: 'highscore_submit.php?name='+encodeURIComponent(_name)+
		         		'&score='+_score+'&hash='+'',
		         		type:'GET'
		         ...
		         showScores:function(){
		         	...
		         	//hide submit graphics and show top ten scores
		         	this.scoreSubmission.visible = false;

		         	//clear out children from previous score checks
		         	while(this.highScoreList.children.length>0){
		         		var child = this.highScoreList.getChildAt(0);
		         		this.highScoreList.removeChild(child);
		         	}

		         	TweenMax.to(this.highScoreList,0.5,{alpha:1});

		         	$.ajax({
		         		url:'display_highscores.php?pagination=0',
		         		type:'GET',
		         		dataType:'json'
		         	}).done(function(d){

		         		$.each(d,function(i,item){
		         		//loop through json array sent from display_highscores.php and display in two rows
		         		var highScoreName = new PIXI.Text(item.name+' > '+item.score, {font:(20*dpr)+'px lateron-Regular', fill:'black'});
		         		ref.highScoreList.addChild(highScoreName);

		         		//position across 2 rows using the modulus at 2, move over every other row
		         		highScoreName.position.x = ((i%2)*150)*dpr;
		         		if(i%2==0){
		         			highScoreName.position.y = (((i-1)*10)+20)*dpr;
		         		}else{
		         			highScoreName.position.y =(i*10)*dpr;
		         		}
		         	});
		         	})
		         }
